...
class _EditProfileViewState extends ConsumerState<EditProfileView> {
  ...
  late final Future<QuerySnapshot<UserModel>> _userInfo;
  late final User _user;

  @override
  void didChangeDependencies() {
    _user = ref.watch(authServicesProvider).getCurrentUser!;
    _userInfo = ref.watch(firestoreServicesProvider).fetchUserWithId(_user.uid);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<QuerySnapshot<UserModel>>(
        future: _userInfo,
        builder: (_, AsyncSnapshot<QuerySnapshot<UserModel>> snapshot) {
          if (snapshot.hasData) {
            final _futureUser = snapshot.data!.docs.first.data();
            ...
            return Padding(
              ...
              child: Column(
                children: [
                  ...
                  Expanded(
                    flex: 6,
                    child: _accountInfoForm(_user, _futureUser),
                  ),
                  Expanded(
                    flex: 1,
                    child: TextButton(
  ...
  Form _accountInfoForm(User? _user, UserModel _futureUser) {
    return Form(
      key: _key,
      child: Column(
        ...
        children: [
          ...
          nextButton(_user, _image, _futureUser),
        ],
      ),
    );
  }
  ...
  ElevatedButton nextButton(User? _user, XFile? image, UserModel _userInfo) {
              ...
              if (_key.currentState!.validate()) {
                try {
                  ...
                  if (image != null) {
                    ...
                    UserModel tempUser = _userInfo.copyWith(
                      id: _user.uid,
                    ...
                    showSnackBar(context, "Select photo");
                  }
                } catch (e) {
                  showSnackBar(context, "Could not update");
                } finally {
                  ref.read(isLoadingProvider.notifier).changeIsLoading();
                }
              }
